...
HRESULT __stdcall DirectInput8Create_Hook(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID *ppvOut, LPUNKNOWN punkOuter)
{
	...
	DirectInput8Hook stHook{};
	stHook.GetDeviceState_Hook = DI8_GetDeviceState_Hook;
	DirectInput8 = new DirectInput8Wrapper(legit, stHook);
	*ppvOut = DirectInput8;
	return result;
}
void Hook_DInput8_DirectInput8Create()
{
	if (DetourFunction(&(PVOID&)DirectInput8Create_Original, DirectInput8Create_Hook))
		logfs("Detours: Hooked second stage DirectInput8Create");
	else
		logfs("Detours: Failed to hook DirectInput8Create");
}